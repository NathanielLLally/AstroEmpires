...
sub listCalls {
  ...
  my @calls;
  no strict 'refs';
  my $name = (defined $obj)?ref($obj):ref($s);
  while (my ($k, $v) = each %{*{"$name\::"}}) {
    if (defined $v && *{$v}{CODE} && $k =~ /^[A-Z]/) {
#      push @calls, sprintf("%s::%s", $name, $k);
      push @calls, sprintf("%s", $k);
    }
  }
  return \@calls;
}
